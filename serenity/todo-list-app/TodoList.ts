...
export class TodoList {
  ...
  static markAsCompleted = (itemNames: Array<Answerable<string>>) =>
    Task.where(
      ...
      ...itemNames.map((itemName) =>
        TodoListItem.markAsCompleted(this.#itemCalled(itemName))
      )
    );
  ...
  static markAsOutstanding = (itemNames: Array<Answerable<string>>) =>
    Task.where(
      ...
      ...itemNames.map((itemName) =>
        TodoListItem.markAsOutstanding(this.#itemCalled(itemName))
      )
    );

  static outstandingItemsCount = () =>
    Text.of(
      PageElement.located(By.tagName("strong")).of(
        this.#outstandingItemsLabel()
      )
    )
      .as(Number)
  ...
  static #outstandingItemsLabel = () =>
    PageElement.located(By.css(".todo-count")).describedAs(
      "items left counter"
    );

  static #newTodoInput = () =>
    PageElement.located(By.css(".new-todo")).describedAs(
      '"What needs to be done?" input box'
    );
  ...
  static #emptyLocalStorage = () =>
    Task.where(
      ...
      ExecuteScript.sync(
        `window.localStorage.removeItem('serenity-js-todo-app')`
      )
    );
}
